/**
 * Script para buscar imágenes en Unsplash y actualizar lessons.ts
 * 
 * La API de Unsplash tiene rate limit de 50 requests/hora para keys gratuitas.
 * Este script hace las búsquedas una por una con pausas y guarda resultados parciales.
 * 
 * Uso: node scripts/replace-images-v2.js
 */
...
async function main() {
    ...
    if (fs.existsSync(resultsPath)) {
        try {
            ...
            console.log('⚠️ No se pudieron cargar resultados previos');
        }
    }
    ...
    for (const [lessonId, query] of lessons) {
        // Saltar si ya tenemos resultado
        if (results[lessonId] && results[lessonId] !== null) {
            console.log(`  [${lessonId}] ✅ Ya buscado - ${results[lessonId].url}`);
            continue;
        }
        ...
        try {
            ...
            // Si es rate limit, guardar y salir
            if (error.message.includes('403')) {
                console.log('\n⚠️ Rate limit alcanzado. Resultados parciales guardados.');
                console.log('   Espera una hora y ejecuta el script de nuevo para continuar.');
                break;
            }
    ...
    for (const [lessonId, result] of Object.entries(results)) {
        if (result) {
            ...
            console.log(`  ${lessonId}: [PENDIENTE]`);
        }
    }
}
